---
description: 'Creación de un cuadro de diálogo: documentos de Marketo, documentación del producto'
title: Crear un cuadro de diálogo
feature: Dynamic Chat
exl-id: 7de6ba05-39d5-41b9-921b-50bf5b2c5581
source-git-commit: f5f93a993d5b13c1fda0b31172393eff0bc65fd4
workflow-type: tm+mt
source-wordcount: '79'
ht-degree: 0%

---

# Crear un cuadro de diálogo {#create-a-dialogue}

A continuación se muestra cómo crear un nuevo Diálogo.

1. Clic **[!UICONTROL Cuadros de diálogo]**.

   ![](assets/create-a-dialogue-1.png)

1. Haga clic en **[!UICONTROL Crear nuevo]** botón.

   ![](assets/create-a-dialogue-2.png)

1. Elija un cuadro de diálogo en blanco o una de las plantillas rellenadas previamente. Introduzca un nombre (la descripción es opcional), cambie el nivel de prioridad (opcional) y haga clic en **[!UICONTROL Crear]**.

   ![](assets/create-a-dialogue-3.png)

>[!NOTE]
>
>La prioridad determina qué cuadro de diálogo se mostrará a un visitante cuando cumpla los requisitos para varios cuadros de diálogo simultáneamente.

A continuación, aprenda a [crear un flujo](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/stream-designer.md#create-a-stream){target="_blank"}.

>[!MORELIKETHIS]
>
>* [Criterios de audiencia](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/audience-criteria.md){target="_blank"}
>* [Diseñador de secuencias](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/stream-designer.md){target="_blank"}
