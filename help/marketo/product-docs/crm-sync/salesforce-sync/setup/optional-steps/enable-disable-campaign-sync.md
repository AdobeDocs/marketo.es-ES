---
unique-page-id: 4719289
description: Obtenga información sobre cómo habilitar o deshabilitar la sincronización de campañas de Salesforce en Marketo Engage.
title: Habilitar/deshabilitar la sincronización de la campaña
exl-id: a8d53268-75b9-479d-8ffc-b3f19eb77483
feature: Salesforce Integration
source-git-commit: d20c398cd1f5ed2646f56995c35a57630c3f2e95
workflow-type: tm+mt
source-wordcount: '85'
ht-degree: 9%

---

# Habilitar/deshabilitar la sincronización de la campaña {#enable-disable-campaign-sync}

>[!NOTE]
>
>**Se requieren permisos de administración**

Esta opción permite a Marketo Engage sincronizar la pertenencia y los estados de sus programas con las campañas de Salesforce y viceversa.

1. Vaya a **[!UICONTROL Admin]** y haga clic en **[!DNL Salesforce]**.

   ![](assets/image2014-12-9-13-3a36-3a49.png)

1. Haga clic en **[!UICONTROL Editar configuración de sincronización]**.

   ![](assets/image2014-12-9-13-3a37-3a0.png)

1. Marque **[!UICONTROL Habilitar sincronización de Salesforce Campaign]** y haga clic en **[!UICONTROL Guardar]**.

   ![](assets/image2014-12-9-13-3a37-3a8.png)

La sincronización puede tardar algún tiempo en extraer los datos de Salesforce.

>[!MORELIKETHIS]
>
>* [Sincronización de SFDC: Sincronización de Campaign](/help/marketo/product-docs/crm-sync/salesforce-sync/sfdc-sync-details/sfdc-sync-campaign-sync.md){target="_blank"}
>* [Establecer apellidos y valores de compañía de posible cliente predeterminados](/help/marketo/product-docs/crm-sync/salesforce-sync/setup/optional-steps/set-default-person-last-name-and-company-name.md){target="_blank"}
>* [Personalizar sincronización de actividades](/help/marketo/product-docs/crm-sync/salesforce-sync/setup/optional-steps/customize-activities-sync.md){target="_blank"}
