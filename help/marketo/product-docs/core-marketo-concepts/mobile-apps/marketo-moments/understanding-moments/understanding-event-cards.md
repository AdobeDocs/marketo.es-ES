---
unique-page-id: 8781974
description: Explicación de las tarjetas de eventos - Documentos de Marketo - Documentación del producto
title: Información sobre las tarjetas de eventos
exl-id: 21e14ede-7b94-4580-8a4c-1b18a3734eef
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '94'
ht-degree: 0%

---

# Información sobre las tarjetas de eventos {#understanding-event-cards}

Utilice Momentos de Marketo para ver los programas de eventos en su teléfono o iPad.

Al pulsar cualquier tarjeta del programa de correo electrónico, puede:

* [Convertir un evento en favorito](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/marketo-moments/working-with-moments/creating-a-favorite.md)
* [Marcar un evento como hecho](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/marketo-moments/working-with-moments/marking-it-done.md)
* [Compartir una tarjeta del momento del evento](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/marketo-moments/working-with-moments/sharing-a-moment.md)

Hay dos tarjetas de eventos. La tarjeta On-Deck, enviada unas horas antes del evento, muestra cuántas personas han iniciado sesión. La tarjeta Resultados, que se envía posteriormente, muestra a cuántas personas asistieron realmente.

![](assets/image2015-7-15-16-3a56-3a16.png)

¡Eres bueno!

>[!MORELIKETHIS]
>
>* [Explicación de los momentos de Marketo](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/marketo-moments/understanding-moments/understanding-marketo-moments.md)
>* [Información sobre las tarjetas de programa de correo electrónico](/help/marketo/product-docs/core-marketo-concepts/mobile-apps/marketo-moments/understanding-moments/understanding-email-program-cards.md)
>* [Explicación de los programas de eventos](/help/marketo/product-docs/demand-generation/events/understanding-events/understanding-event-programs.md)

