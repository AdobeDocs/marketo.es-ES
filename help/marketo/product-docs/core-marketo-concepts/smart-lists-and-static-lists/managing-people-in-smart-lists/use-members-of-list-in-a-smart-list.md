---
unique-page-id: 2953173
description: Aprenda a utilizar miembros de listas en un filtro de listas inteligentes. Incluir o excluir a personas que están en otra lista.
title: Uso de miembros de una lista en una lista inteligente
exl-id: ebd458c1-01f4-4d19-b5b2-d19ab2bad1af
feature: Smart Lists
source-git-commit: 60c5603fa29bb1039b9d477633beb2c6f5c63486
workflow-type: tm+mt
source-wordcount: '144'
ht-degree: 11%

---

# Uso de miembros de una lista en una lista inteligente {#use-members-of-list-in-a-smart-list}

>[!TIP]
>
>Puede agregar personas a una lista mediante [Importar](/help/marketo/getting-started/quick-wins/import-a-list-of-people.md) o el paso de flujo [Agregar a la lista](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/flow-actions/add-to-list.md){target="_blank"}.

Con este filtro, puede extraer miembros de otra lista haciendo referencia a ellos en las reglas de listas inteligentes.

1. Seleccione una lista inteligente y haga clic en la ficha **[!UICONTROL Lista inteligente]**.

   ![](assets/smartlist-sltab.png)

1. En el panel de filtros del lado derecho, busque y arrastre el filtro **[!UICONTROL Miembro de la lista]** al lienzo.

   ![](assets/use-members-of-list-in-a-smart-list-2nd.png)

1. Haga clic en la lista desplegable o escriba para buscar la lista que desee incluir en la lista inteligente.

   ![](assets/memberoflist.png)

   En este ejemplo, la lista inteligente ahora se dirigirá únicamente a los miembros de esa lista y los evaluará en función de cualquier otra regla que incluya.
