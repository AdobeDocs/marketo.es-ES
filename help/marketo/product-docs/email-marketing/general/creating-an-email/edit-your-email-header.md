---
unique-page-id: 2359453
description: 'Editar el encabezado del correo electrónico: Documentos de Marketo: Documentación del producto'
title: Editar el encabezado del correo electrónico
exl-id: 21c529d1-55b0-4529-ac0a-4dfb9d149686
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '195'
ht-degree: 0%

---

# Editar el encabezado del correo electrónico {#edit-your-email-header}

El encabezado del correo electrónico en Marketo es totalmente personalizable. Consta de cuatro campos:

* **De** - El nombre del remitente tal como desea que aparezca
* **De la dirección** - La dirección de correo electrónico del remitente tal como desea que aparezca
* **Responder** - La dirección de correo electrónico a la que desea que se envíe la respuesta de una persona (puede ser diferente a De dirección)
* **Asunto** - La línea de asunto del correo electrónico

Para editar estos valores, haga clic en cada campo e introduzca la información.

![](assets/one-3.png)

>[!TIP]
>
>Para establecer un valor predeterminado de Desde nombre y Desde correo electrónico, consulte [Cambiar el valor predeterminado de Correo electrónico y de Etiqueta](/help/marketo/product-docs/administration/email-setup/change-the-default-from-email-and-from-label.md).

Si desea utilizar un token, haga clic primero dentro del campo deseado y, a continuación, haga clic en el icono del token.

![](assets/two-3.png)

También puede hacer que el campo sea dinámico utilizando segmentos.

![](assets/three-2.png)

El icono de clave situado al extremo derecho en el campo De dirección le permite saber si está utilizando una firma DKIM personalizada.

![](assets/four-2.png)

El contador situado en el extremo derecho del campo Asunto le ayuda a mantener la línea de asunto por debajo del límite recomendado de 50 caracteres.

![](assets/five-1.png)

Si supera los 50 caracteres, el contador se vuelve rojo para avisarle.

![](assets/six-1.png)

>[!MORELIKETHIS]
>
>[Información general del Editor de correo electrónico v2.0](/help/marketo/product-docs/email-marketing/general/email-editor-2/email-editor-v2-0-overview.md)
