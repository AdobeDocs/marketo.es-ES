---
unique-page-id: 14352601
description: Aprenda a hacer referencia a sí mismo en un campo dinámico de Sales Connect. Utilice el nombre o los detalles del remitente en la personalización de plantillas.
title: Hacerse referencia a sí mismo en un campo dinámico
exl-id: 89d5ad5e-e699-451c-b399-f3cc7f554100
feature: Marketo Sales Connect
TQID: https://experienceleague.adobe.com/dNvB4eEnqeOYCSvNzK6DOdxhAg-1u-FwyuvI-WTElUQ
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: f82558ea-6af5-44eb-a424-5b3389abb0a3
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 93
ht-degree: 12%

---

# Hacerse referencia a sí mismo en un campo dinámico {#reference-yourself-in-a-dynamic-field}

Los campos dinámicos son nuestra forma de combinación de correspondencia.

Le permitimos personalizar sus plantillas de correo electrónico con atributos predefinidos como `{{my_name}}` o `{{my_title}}`. Estos campos le permiten hacer referencia rápidamente a sí mismo en sus plantillas de correo electrónico.

Para agregar un campo dinámico de referencia automática a una plantilla, simplemente seleccione el que desee usar en el botón **[!UICONTROL Campos dinámicos MSE]** del editor de plantillas.
