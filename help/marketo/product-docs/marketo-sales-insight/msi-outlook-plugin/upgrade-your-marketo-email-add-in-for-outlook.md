---
unique-page-id: 2949279
description: 'Actualizar el complemento de correo electrónico de Marketo para  [!DNL Outlook] : documentos de Marketo: documentación del producto'
title: Actualizar el complemento de correo electrónico de Marketo para  [!DNL Outlook]
exl-id: 079f1142-8062-448c-aa07-59ecd89a718f
feature: Marketo Sales Insights
source-git-commit: 0d37fbdb7d08901458c1744dc68893e155176327
workflow-type: tm+mt
source-wordcount: '326'
ht-degree: 11%

---

# Actualizar el complemento de correo electrónico de Marketo para [!DNL Outlook] {#upgrade-your-marketo-email-add-in-for-outlook}

Cuando haya disponible una nueva versión del complemento de correo electrónico de Marketo para [!DNL Outlook], siga estas instrucciones para actualizar.

>[!NOTE]
>
>A partir del 1/10/2020, la última versión del complemento [!DNL Outlook] ya no será compatible con el modo sin conexión. Esto surtirá efecto después de realizar la instalación o la actualización el 1/10 o después.

## Descargar instalador {#download-installer}

Descargue el instalador apropiado para su versión de Microsoft [!DNL Outlook].

<table> 
 <colgroup> 
  <col> 
  <col> 
  <col> 
  <col> 
  <col> 
 </colgroup> 
 <tbody> 
  <tr> 
   <th><br></th> 
   <th colspan="2">Instalación de invitación de usuario único</th> 
   <th colspan="2">Instalación de clave empresarial</th> 
  </tr> 
  <tr> 
   <td><strong>Versión de <span class="dnl">Outlook</span></strong></td> 
   <td><strong>de 32 bits</strong></td> 
   <td><strong>de 64 bits</strong></td> 
   <td><strong>de 32 bits</strong></td> 
   <td><strong>de 64 bits</strong></td> 
  </tr> 
  <tr> 
   <td><span class="dnl">Outlook</span> 2000</td> 
   <td>No compatible</td> 
   <td>N/A</td> 
   <td>No compatible</td> 
   <td>N/A</td> 
  </tr> 
  <tr> 
   <td><span class="dnl">Outlook 2003</span></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td>N/A</td> 
   <td>No compatible</td> 
   <td>N/A</td> 
  </tr> 
  <tr> 
   <td><span class="dnl">Outlook</span> 2007</td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td>N/A</td> 
   <td>No compatible</td> 
   <td>N/A</td> 
  </tr> 
  <tr> 
   <td><span class="dnl">Outlook</span> 2010</td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
  </tr> 
  <tr> 
   <td><span class="dnl">Outlook</span> 2013</td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
  </tr> 
  <tr> 
   <td><span class="dnl">Outlook</span> 2016</td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
  </tr> 
  <tr> 
   <td colspan="1"><span class="dnl">Outlook</span> 2019</td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
  </tr> 
  <tr> 
   <td><span class="dnl">Outlook</span> para Mac</td> 
   <td>No compatible</td> 
   <td>No compatible</td> 
   <td>No compatible</td> 
   <td>No compatible</td> 
  </tr> 
  <tr> 
   <td colspan="1"><span class="dnl">Outlook</span> aplicación web</td> 
   <td colspan="1">No compatible</td> 
   <td colspan="1">No compatible</td> 
   <td colspan="1">No compatible</td> 
   <td colspan="1">No compatible</td> 
  </tr> 
  <tr> 
   <td colspan="1"><span class="dnl">Oficina</span> 365*</td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup32.msi" rel="nofollow">Descargar</a></td> 
   <td colspan="1"><a href="https://munchkin.marketo.net/MarketoAddInSetup64.msi" rel="nofollow">Descargar</a></td> 
  </tr> 
 </tbody> 
</table>

&#42;Versión de Office 365: solo cliente de Windows (en Windows 10, Windows 11, Enterprise o Pro).

## Actualizar {#upgrade}

1. Identifique su [[!DNL Microsoft Outlook] versión](https://support.microsoft.com/en-us/office/what-version-of-outlook-do-i-have-b3a9568c-edb5-42b9-9825-d48d82b2257c?ui=en-us&rs=en-us&ad=us).

1. Seleccione su versión en la lista anterior.

1. Ejecute el instalador.

   ![](assets/image2014-9-23-16-3a53-3a56.png)

1. Haga clic en **[!UICONTROL Siguiente]**.

   ![](assets/image2014-9-23-16-3a54-3a8.png)

   >[!NOTE]
   >
   >En algunos casos, faltan los datos. Cópielo del correo electrónico de registro y luego cierre [!DNL Outlook].

1. Cerrar [!DNL Microsoft Outlook].

   ![](assets/ent-key-close-outlook-hand.png)

1. Verá que toda su información está rellenada previamente. Simplemente haga clic en **[!UICONTROL Siguiente]**.

   ![](assets/image2014-9-23-16-3a54-3a40.png)

   >[!TIP]
   >
   >Si la instalación falla, colabore con su departamento de TI para asegurarse de que el tráfico HTTPS no esté bloqueado. El instalador requiere que el tráfico HTTPS esté abierto.

1. Haga clic en **[!UICONTROL Siguiente]** para instalarlo en la ubicación predeterminada.

   ![](assets/image2014-9-23-16-3a54-3a55.png)

1. Haga clic en **[!UICONTROL Siguiente]**.

   ![](assets/image2014-9-23-16-3a55-3a20.png)

1. La instalación ha finalizado. Haga clic en **[!UICONTROL Cerrar]**.

   ![](assets/image2014-9-23-16-3a55-3a34.png)

1. Ahora abre [!UICONTROL Microsoft Outlook] para ver la última versión de los botones de Marketo.

   ![](assets/image2016-8-24-15-3a47-3a38.png)

>[!MORELIKETHIS]
>
>* [Enviar y rastrear un correo electrónico con el complemento de correo electrónico de Marketo para [!UICONTROL Outlook]](/help/marketo/product-docs/marketo-sales-insight/msi-outlook-plugin/send-and-track-an-email-with-the-email-add-in-for-outlook.md)
>* [Enviar y rastrear desde [!UICONTROL Outlook] con una plantilla de Marketo](/help/marketo/product-docs/marketo-sales-insight/msi-outlook-plugin/send-and-track-from-outlook-using-a-marketo-template.md)
